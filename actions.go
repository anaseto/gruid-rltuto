...
func (m *model) UpdateFOV() {
	...
	player.FOV.SetRange(rg.Add(player.P).Intersect(m.game.Map.Grid.Range()))
	...
	for _, p := range player.FOV.SSCVisionMap(player.P, maxLOS, passable, false) {
		if paths.DistanceManhattan(p, player.P) > maxLOS {
			continue
		}
